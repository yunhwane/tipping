...
export const userRouter = createTRPCRouter({
  getProfile: protectedProcedure.query(async ({ ctx }) => {
    const user = await ctx.db.user.findUnique({
      ...
      select: { id: true, name: true, email: true, image: true, bio: true, links: true, role: true },
    });
    if (!user) {
  ...
  updateProfile: protectedProcedure
    .input(
      z.object({
        name: z.string().min(2, "닉네임은 2자 이상이어야 합니다").max(20, "닉네임은 20자 이하여야 합니다").transform((v) => v.trim()).optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      const data: { name?: string; image?: string; bio?: string | null; links?: { label: string; url: string }[] } = {};
